  ...
  <div class="type-wrapper">
    ...
    <section class="list">
      <ul @click="tabClick" class='type-left'>
        ...
        <!-- 进行数据渲染 -->
        <li v-for="(item, index) in categories" :key="index" :class="index==current?'active':''" :data-index="index">
          <!-- 制造一个假的form提交表单，用户每次点击事件就生成伪造 -->
  ...
  export default {
    ...
    computed: {
      ...
      currentList() {
        ...
        if (this.categories.length) {
          ...
          // 12.替换所切换跳转的路径
          current.cover_url = current.cover_url.replace(
            "/commodity/details",
            "/detail/main"
          );
          // 13.将替换好的路径返出
    ...
    methods: {
      ...
      tabClick(e) {
        ...
        if (
          e.target.dataset.index != undefined && e.target.dataset.index != this.current
        ) {
